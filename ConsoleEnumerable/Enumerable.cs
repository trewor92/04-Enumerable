...
namespace ConsoleEnumerable
{
    ...
    public class Task
    {
        ...
        {
            return data.Select(x => x == null ? null : x.ToUpper());
        ...
        public IEnumerable<long> GetMovingSumSequence(IEnumerable<int> data)
        {
            ...
            return data.Select(x => sum += (long)x);
        ...
        public IEnumerable<string> GetPrefixItems(IEnumerable<string> data, string prefix)
        {
            ...
            return data.
                Where(x => x == null ? false : x.IndexOf(prefix, StringComparison.OrdinalIgnoreCase) >= 0);
        ...
        {
            return data.FirstOrDefault(x => x == null ? false : x.IndexOf("first", StringComparison.OrdinalIgnoreCase) > 0);
        ...
        {
            return data.Distinct().Count(x => x == null ? false : x.Length == 3);
        ...
        {
            return data.GroupBy(x => x).Select(x => Tuple.Create(x.Key, x.Count()));
        ...
        {
            return data.OrderByDescending(x => x != null ? x.Length : 0)
                .GroupBy(x => x != null ? x.Length : 0)
        ...
        public IEnumerable<string> GetIEnumerableTypesNames(Assembly assembly)
        {
            ...
            return assembly.ExportedTypes.
                Where(x => typeof(System.Collections.IEnumerable).IsAssignableFrom(x)).Select(x => x.Name);
        ...
        public int[] GetQuarterSales(IEnumerable<Tuple<DateTime, int>> sales)
        {
            return sales.Aggregate(new int[4],
                (array, x) => { array[GetQuater(x.Item1) - 1] += x.Item2; return array; },
                result => result);
            // TODO : Implement GetQuarterSales
            //throw new NotImplementedException();
        }

        private int GetQuater(DateTime dateTime)
        {
            if (dateTime == null)
                throw new ArgumentNullException();
        ...
        {
            return  data.OrderBy(x => x == null ? 0 : x.Length).ThenBy(x => x);
        ...
        public IEnumerable<string> SortDigitNamesByNumericOrder(IEnumerable<string> data)
        {
            ...
            return data.Select(x => Array.IndexOf(numbers, x)).OrderBy(x => x).Select(x => numbers[x]);
        ...
        {
            return boys.Where(b => b != null)
                .SelectMany(b => girls.Where(g => g != null).Select(g => b + "+" + g ));
